...
int G1 = 270, G2 = 350, G3 = 400, G4 = 300;
int counter1 = 0, counter2 = 1, counter3 = 1;
...
vector <HIGH_SCORES> topScores;
sf::Event evnt;
Position pacPos,ghost1,ghost2,ghost3,ghost4,pac2Pos;
int currPage = 1;
int dots=0, dots1 =290,dots2=308,dots3=336; 
...
int main()
{
	...
	sf::RenderWindow window(sf::VideoMode(1300,700), "Pacman", sf::Style::Close | sf::Style::Titlebar|sf::Style::Fullscreen);
	...
	sf::RenderWindow mainMenuWindow(sf::VideoMode(1300, 800), "ANGRY-MAN", sf::Style::Fullscreen);

	sf::RectangleShape photo(sf::Vector2f(1300.0f, 800.0f));
	...
	while(window.isOpen())
	{
		while(window.pollEvent(evnt))
		{
			...
			else if (evnt.type==evnt.KeyPressed)
			{
				...
				else if(evnt.key.code==sf::Keyboard::P && start)
				{
					start=false;
		...
		while(mainMenuWindow.pollEvent(evnt))
		{
			...
			{	
				...
				else if (evnt.key.code==sf::Keyboard::Return && choice==0)
				{
					currPage=2;
					page2.requestFocus();
				}
			...
			if (evnt.type==evnt.KeyPressed)
			{
				if(evnt.key.code==sf::Keyboard::Return)
				{
					if(page==2)
					{
						...
						page2.setSize(sf::Vector2u(750,400));
						page++;
						if (selectedmode==-1)
							selectedmode=0;
						if(selectedmode==0)
				...
				{
					if(characterIndex==0)
						selectCharacter(evnt.key.code,character,-1);
					else if(characterIndex==1)
						selectCharacter(evnt.key.code,character2,character);
				}
			}
		}
		while(messageWindow.pollEvent(evnt))
		{
			if (evnt.type==evnt.KeyPressed)
			{
		...
		while(ScoreWindow.pollEvent(evnt))
		{
			...
			else if (evnt.type == sf::Event::TextEntered)
			{
				...
				if (evnt.text.unicode == '\b' && playerInput.getSize()>1)
					playerInput.erase(playerInput.getSize()- 1, playerInput.getSize());
				else if(evnt.text.unicode < 128 && evnt.text.unicode!= 13)
		...
		if(twoplayers && (scorePLAYER1+scorePLAYER2>=score3|| livesPL1<=0|| livesPL2<=0))
		{
		...
		{
			if(score==score1+score2+score3 && YouWin)
			{
				YouWin=false;
				messageWindow.clear(sf::Color::Black);
			...
			else if (score==score1 || score==score1+score2)
			{
				start=false;
				messageWindow.clear(sf::Color::Black);
			...
			if(score==score1+score2+score3 && soundDuration)
			{
				soundDuration=false;
				musicwin.play();
			}
			...
			if(score==score1 || score==score1+score2)
				musiclevel.play();
			levels++;
			messageWindow.display();
			resetMaze();
			...
			window.requestFocus();
			start=true;
			if(score==score1+score2+score3 && isHighScore(score))
			{
				enterName=false;
				currPage = 6;
...
void loadScore(vector <HIGH_SCORES>& topScores)
{
	...
	while (getline(infile, line))
	{
		...
		if (pos!=string::npos)
		{
			...
			sscanf(s.c_str(),"%i",&compare.score);
...
void saveScore(vector <HIGH_SCORES>& topScores,String Input,int score5)
{
	...
	int n = __min(5, topScores.size()+1);
	FILE *obj=fopen("thefile.txt","w");
	bool inserted = false;
	for (int i=0;i<n;i++)
	{
		if (topScores[i].score<=score5 && !inserted)
		{
			inserted = true;
...
void drawNameAndScore(vector <HIGH_SCORES>& topScores ,sf::RenderWindow& leaderborad)
{
	...
	{
		sprintf(line,"%i       %s",i+1,topScores[i].name);
		sf::Text linee;
		linee.setFont(font);
		...
		sprintf(line,"%i",topScores[i].score);
		sf::Text scr;
		scr.setFont(font);
		scr.setString(line);
...
void resetGame(void)
{
	dots=0, dots1 =290,dots2=308,dots3=336; 
	pacPos.x=1;
	pacPos.y=1;
	if (selectedmode==0)
	{
